...
### _catalog Field Requirements

`_catalog` is the source of truth for this dataset's entry on the website data
page. `scripts/build_docs.R` reads it to generate `docs/data_sources_index.json`
— see **Updating the Data Sources Index** below for what each field means and
how to write it.
...
`summary` fields were written as `<U+2013>`. Run the script with
`LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8 Rscript scripts/build_docs.R`; the index is
regenerated from the `_catalog` blocks in `measure_info.json`, so a rebuild in a
UTF-8 locale fixes it (check the `_catalog.summary` source text too).
...
## Updating the Data Sources Index

`docs/data_sources_index.json` is a lightweight, per-source catalog used by the
website data page. It is **fully generated** by `scripts/build_docs.R` alongside
the HTML docs and manifest, so it refreshes on every build (including the daily
GitHub Action). **Never hand-edit it.**

The hand-written text lives in each source's `measure_info.json`, under a
top-level `_catalog` block:
...
"_catalog": {
  ...
  "search_terms": ["Respiratory", "flu", "rsv"],
  "bucket": [],
  "files": { "data.csv.gz": "Vaccination uptake by vaccine and age" }
}
```

| Index field | Comes from | Notes |
|-------------|------------|-------|
| `dataset`, `github_folder`, `data_url`, `data_dictionary`, `latest_date` | computed from the repo on every build | never editable |
| `files[].dataset_link` | computed (direct raw csv.gz link) | never editable |
| `name` | `_catalog.name`, else `_sources[].name` | omit unless a more specific label is needed (e.g. the `epic_*` dirs, which otherwise all read "Epic Cosmos") |
| `summary` | `_catalog.summary` | 1–2 sentences, ~25 words, 15-year-old reading level |
| `search_terms` | `_catalog.search_terms` | website search box; ~3–4 terms, capitalized topic label(s) first, then lowercase specifics |
| `bucket` | `_catalog.bucket` | site-navigation grouping; currently `[]` for every dataset |
| `files[].dataset_stratification` | `_catalog.files["<file name>"]` | keyed by bare file name; ~5-word noun phrase |

### Defaults when a `_catalog` field is absent

- `name` → `_sources[].name`.
- `summary` → a mechanical extractive stub (first sentence of the `_sources`
  descriptions), plus a build warning (below).
- `search_terms` and `bucket` → both from bundle membership
  (`bundle_chronic_diseases` → `"Chronic diseases"`). They are two independent
  fields: deleting one from `_catalog` re-derives only that one, and an explicit
  `[]` is respected as an intentional "none" and sticks.
- a file with no key in `_catalog.files` → a blurb derived from the file name
  (`"Stratified by <tokens>."` / `"Overall; no stratification beyond time and
  geography."`). Both are placeholders, never ship them.

**Warning — never blank a `summary` and run `build_docs.R` on its own.** A
blanked `_catalog.summary` falls back to the extractive stub, which reads as an
abrupt, often mid-sentence fragment — this has already happened once and
silently flattened nearly every dataset's hand-written summary in a single
commit. The sequence must always be: write the real replacement text into
`_catalog` (or use the `update-data-sources-index` skill, which does this for
you) **before** running `Rscript scripts/build_docs.R` — never
blank-then-rebuild-then-fill. `build_docs.R` prints `WARNING: <dataset> has no
hand-written summary in measure_info.json _catalog` whenever it falls back,
precisely so this regression is visible in build/CI logs instead of slipping
through silently — treat that warning line as a signal to stop and write a real
summary before committing.

### After ingesting a new dataset

1. **Write `_catalog`** into `data/<source_name>/measure_info.json` — `summary`,
   `search_terms`, `bucket`, and one `files` entry per `standard/*.csv.gz`.
2. **Rebuild** from the repo root:
   ```r
   Rscript scripts/build_docs.R
   ```
3. **Verify** the generated entry in `docs/data_sources_index.json` and **commit**
   `measure_info.json` together with the regenerated `docs/` and
   `resources/data_manifest.json`.

The `update-data-sources-index` skill automates steps 1–3 (it writes the summary
and stratification text for you, then rebuilds and verifies).
