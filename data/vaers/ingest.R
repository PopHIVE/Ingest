...
symptoms <- lapply(allyears, function(x) read_csv_arrow(paste0('./raw/staging/',x,'VAERSSYMPTOMS.csv')) ) %>%
  bind_rows()

arrow::write_dataset(symptoms,path='./raw/symptoms.parquet', format='parquet' ,max_rows_per_file=50000)

#VAERS Vaccine files
vax <- lapply(allyears, function(x) read_csv(paste0('./raw/staging/',x,'VAERSVAX.csv')) ) 

vax <- map_dfr(vax, ~ mutate_all(.x, as.character)) #convert everything to character

vax <-  bind_rows(vax)

arrow::write_dataset(vax,path='./raw/vax.parquet', format='parquet' ,max_rows_per_file=50000)



#VAERS data files
vaersdata <- lapply(allyears, function(x) read_csv(paste0('./raw/staging/',x,'VAERSDATA.csv')) )

vaersdata <- map_dfr(vaersdata, ~ mutate_all(.x, as.character)) #convert everything to character

vaersdata <- bind_rows(vaersdata)

arrow::write_dataset(vaersdata, path='./raw/vaersdata.parquet', format='parquet' ,max_rows_per_file=10000)
