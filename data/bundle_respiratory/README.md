...
Two of the `dist/` parquets cover streptococcal surveillance (previously the
separate `bundle_strep`), combining three sources:

| Parquet | Source | Rows | Grain |
|---|---|---|---|
| `abcs_strep.parquet` | `abcs` | 6,101 | national, annual — all eight ABCs strep topics stacked |
| `gas_state.parquet` | `epic_resp_infections`, `nnds` | 64,480 | state + national, quarterly and weekly |
...
All eight ABCs strep topics (rates, counts, resistance, Group A syndromes and emm
...
### Notes for anyone reading these two files
