# =============================================================================
# OPTIMIZED V2 - FULL SCALE CMS MMD DOWNLOADER (MORE ROBUST)
# =============================================================================

# ... (all library and setup code remains the same) ...
library(httr2)
library(dplyr)
library(glue)
library(purrr)
...
# --- Setup, Labels, and Conditions Map are unchanged ---
# ... (paste your setup code here) ...
base_url <- "https://data.cms.gov/data-api/v1/mmd-tool/"
output_dir <- "raw/cms_mmd_full_scale_optimized"
...
full_scale_download_condition_optimized <- function(condition_code, condition_name, years = 2023) {
  ...
  # =============================================================================
  # FIX: MOVE HELPER FUNCTIONS INSIDE TO ENSURE THEY ARE EXPORTED TO WORKERS
  # =============================================================================
...
for(j in 1:length(all_conditions) ){
  ...
  year_data <- lapply(c(2020:2024), function(i){
     full_scale_download_condition_optimized(
    condition_code = all_conditions[[j]]$code, 
    condition_name = all_conditions[[j]]$name, 
    years = i
  )})
  
  condition_data[[j]] <- bind_rows(year_data)
  vroom::vroom_write(condition_data[[j]], paste0(output_dir,'/stage_',all_conditions[[j]]$name,'.csv.xz'))
}
