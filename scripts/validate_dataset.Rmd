...
classify_columns <- function(d) {
  ...
  # A numeric measure column that is 100% NA for every row written so far
  # (e.g. a brand-new source whose only period is fully suppressed) round-trips
  # through CSV as logical, since there is no non-NA value left to type it by.
  # Treat those the same as numeric so they aren't silently dropped from roles.
  is_measure_like <- function(x) is.numeric(x) || (is.logical(x) && all(is.na(x)))
  measure_cols <- measure_cols[vapply(d[measure_cols], is_measure_like, logical(1))]
...
if (!map_ok) {
  ...
  # one representative time per year, so year labels used for faceting are
  # never duplicated even for sub-annual (e.g. monthly/weekly) data
  all_times <- sort(unique(dat$time))
  all_years <- substr(all_times, 1, 4)
  chosen_years <- sample(unique(all_years), min(4, length(unique(all_years))))
  map_times <- sort(vapply(chosen_years, function(y) sample(all_times[all_years == y], 1), character(1)))
