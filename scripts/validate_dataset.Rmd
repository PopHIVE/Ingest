...
params:
  ...
  data_file: "C:/Users/as5325/Desktop/Ingest/data/respnet/standard/data.csv.gz"
...
has_true_overall <- function(v) any(na.omit(v) %in% OVERALL_LABELS)
...
if (length(state_codes) == 0) {
  ...
  if (file.exists(fips_path)) {
    ...
    lab <- ifelse(is.na(nm), pick, nm)
  }

  for (i in seq_along(pick)) {
    cat(sprintf("<h2>%s</h2>\n\n", lab[i]))
    st_i <- dat %>% filter(geography == pick[i])
    for (col in demo_cols) {
      if (length(unique(st_i[[col]])) < 2) next
      sub <- hold_overall(st_i, setdiff(demo_cols, col))
      cat(sprintf("<h3>by %s</h3>\n\n", col))
      trend_plot(sub, col, "")
    }
  }
}
...
if (!nzchar(cmp_path) || !file.exists(cmp_path)) {
  ...
  if (nrow(pairs) == 0) {
    ...
    cmp_measure_info <- NULL
    cmp_mi_path <- file.path(dirname(dirname(cmp_path)), "measure_info.json")
    if (file.exists(cmp_mi_path)) {
      cmp_measure_info <- tryCatch(jsonlite::read_json(cmp_mi_path), error = function(e) NULL)
    }
    cmp_short_name_fn <- function(v) {
      if (!is.null(cmp_measure_info) && !is.null(cmp_measure_info[[v]]) &&
          !is.null(cmp_measure_info[[v]]$short_name))
        cmp_measure_info[[v]]$short_name else v
    }
